...
```
  Define            Build            Ship            Review           Plan
┌─────────┐    ┌─────────────┐    ┌──────┐    ┌────────────┐    ┌─────────┐
│   PRD   │ ─► │ Prototyper  │ ─► │ Git  │ ─► │  Feedback  │ ─► │ Planner │
│  Skill  │    │   Skill     │    │ Push │    │   Skill    │    │  Export │
└─────────┘    └─────────────┘    └──────┘    └────────────┘    └─────────┘
                     ▲                              │
                     └──────── Revise ──────────────┘
```
...
| 6 | `/planner-export` | PRD → importable CSV for Microsoft Teams Planner |
...
This file is the input for both the prototyping skill and the planner export skill.
...
## 6. Create a Project Plan from the PRD

The planner export skill converts a PRD into a structured CSV that can be imported into Microsoft Teams Planner to auto-create task cards with buckets, priorities, and checklists.
